...
The new broad class included such classes of models as as activity-based travel, cell-based models, and multi-agent models, and more recently comprehensive urban microsimulation models that fully reflect the dynamics of changes in the population and the urban environment\cite{Iacono2008}.
...
The Integrated Land Use, Transportation, Environment (ILUTE) model system is a system of disaggregate agent-based microsimulation models for greater Toronto-Hamilton area.
...
The system uses disaggregate models of spatial socioeconomic processes to evolve from a known base case to a predicted end state of the greater Toronto \textendash Hamilton area in 1-year time steps\cite{Miller2011}.

In ILUTE, the system state of greater Toronto-Hamilton area is defined in terms of the individual persons, households, dwelling units, firms, etc.
...
Continuing methodological advances such as cost-effective High Performance Computing (HPC), detailed GIS-based datasets and machine learning methods mean that former barriers now represent opportunities for model system development\cite{Miller2018a}.
...
Due to its high resolution, Teranet's dataset presents a valuable resource to inform and validate the design of ILUTE and HoMES .
...
\section{Teranet's dataset of land registration records and its challenges} \label{sec:teranet_and_its_challgenges}

\subsection{Longitudinal Housing Market Research conducted by UTTRI} \label{subsec:longitudinal_housing_market_research}
...
\subsection{POLARIS: electronic system of land registration in Ontario} \label{subsec:polaris}
...
\subsection{Teranet's dataset, its challenges and the proposed solution} \label{subsec:teranet_challenges_solution}
...
\begin{itemize}
    \item DMTI's land use data does not offer any split between categories of residential properties and only covers the period of 2001--2014
